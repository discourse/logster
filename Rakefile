...
module DevServer
  ...
  def check_pnpm!
    available = system("pnpm", "--version", out: File::NULL, err: File::NULL)
    abort "pnpm is required. Run `npm install -g pnpm`, then try again." unless available

    puts "✓ pnpm is available"
  rescue Errno::ENOENT
    abort "pnpm is required. Run `npm install -g pnpm`, then try again."
  end
  ...
  def install_frontend_dependencies!
    installed = File.join(CLIENT_DIR, "node_modules", ".pnpm", "lock.yaml")
    lockfile = File.join(CLIENT_DIR, "pnpm-lock.yaml")
    if File.exist?(installed) && File.read(installed) == File.read(lockfile)
      return puts("✓ Frontend dependencies are current")
    end

    puts "Installing frontend dependencies..."
    unless system("pnpm", "install", "--dir", CLIENT_DIR)
      abort "Frontend dependency installation failed"
    end
