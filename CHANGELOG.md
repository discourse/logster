...
- Unreleased
  ...
  - UX: Drive the log search from the `search` query parameter so a search can be linked to and reloaded
  ...
  - DEV: Upgrade the client from Ember 3.28 to Ember 7.2 and serve the scripts and stylesheets listed in a manifest generated by the Ember build
  - DEV: Convert the client components and route templates to Glimmer GJS
  ...
  - DEV: Add a reusable sample-data loader and richer development fixtures, and run the website tests in CI
  - DEV: Ship only the runtime files and the built frontend assets in the gem, along with the third-party license notices webpack extracts, and verify the packaged assets before a release
